...
vec3 CalcPointLightContribution(vec3 worldPos, vec3 normal, vec3 viewDir, Light light, float key,float shininess) {
	...
	//(diffuseOut + specularOut) * attenuation
}
...
*/
vec3 CalcAllLightContribution(vec3 worldPos, vec3 normal, vec3 camPos,float key, float shininess) {
	...
	for(int ix = 0; ix < AmbientColAndNumLights.w && ix < MAX_LIGHTS; ix++) {
		...
		lightAccumulation += CalcPointLightContribution(worldPos, normal, viewDir, Lights[ix], key,shininess);
	}
