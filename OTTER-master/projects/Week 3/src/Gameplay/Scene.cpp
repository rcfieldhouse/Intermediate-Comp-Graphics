...
namespace Gameplay {
	...
	void Scene::SetColorLUT(const Texture3D::Sptr& texture,const int lutNum) {
		if (lutNum == 0) {
			_colorCorrectionCustom = texture;
		}
		if (lutNum == 1) {
			_colorCorrectionWarm = texture;
		}
		if (lutNum == 2) {
	...
	const Texture3D::Sptr& Scene::GetColorLUT() const {
		...
		if (application.userInput() == 8 ) {
			return _colorCorrectionCool;
		}
		else if (application.userInput() == 9) {
			return _colorCorrectionWarm;
		}
		else if (application.userInput() == 0) {
