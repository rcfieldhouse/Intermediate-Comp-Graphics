...
void DefaultSceneLayer::_CreateScene()
{
	...
	if (loadScene && std::filesystem::exists("scene.json")) {
		...
		Texture3D::Sptr lutwarm = ResourceManager::CreateAsset<Texture3D>("luts/cool.CUBE");
		Texture3D::Sptr lutcool = ResourceManager::CreateAsset<Texture3D>("luts/warm.CUBE");


		// Configure the color correction LUT
		scene->SetColorLUT(lutcustom,0);
		scene->SetColorLUT(lutwarm, 1);
		scene->SetColorLUT(lutcool, 2);
		...
		GameObject::Sptr monkey1 = scene->CreateGameObject("Monkey 1");
		{
			...
			monkey1->SetPostion(glm::vec3(3.5f, 2.0f, 1.0f));
			...
			RotatingBehaviour::Sptr jumper1 = monkey1->Add<RotatingBehaviour>();
		...
		GameObject::Sptr monkey2 = scene->CreateGameObject("Monkey 2");
		{
			...
			RotatingBehaviour::Sptr jumper2 = monkey2->Add<RotatingBehaviour>();
			jumper2->setDirection(5);
		...
		{
			// Set position in the scene
			monkey3->SetPostion(glm::vec3(5.5f, 0.0f, 1.0f));

			// Add some behaviour that relies on the physics body
			monkey3->Add<JumpBehaviour>();
			// Create and attach a renderer for the monkey
			RenderComponent::Sptr renderer = monkey3->Add<RenderComponent>();
			renderer->SetMesh(sphere);
			renderer->SetMaterial(boxMaterial);

			RotatingBehaviour::Sptr jumper3 = monkey3->Add<RotatingBehaviour>();
			jumper3->setDirection(4);

			// Example of a trigger that interacts with static and kinematic bodies as well as dynamic bodies
			TriggerVolume::Sptr trigger = monkey3->Add<TriggerVolume>();
			trigger->SetFlags(TriggerTypeFlags::Statics | TriggerTypeFlags::Kinematics);
			trigger->AddCollider(BoxCollider::Create(glm::vec3(1.0f)));

			monkey3->Add<TriggerVolumeEnterBehaviour>();
		}
		GameObject::Sptr monkey4 = scene->CreateGameObject("Monkey 4");
		{
			// Set position in the scene
			monkey4->SetPostion(glm::vec3(1.5f, 4.0f, 1.0f));

			// Add some behaviour that relies on the physics body
			monkey4->Add<JumpBehaviour>();
			// Create and attach a renderer for the monkey
			RenderComponent::Sptr renderer = monkey4->Add<RenderComponent>();
			renderer->SetMesh(sphere);
			renderer->SetMaterial(boxMaterial);
			RotatingBehaviour::Sptr jumper4 = monkey4->Add<RotatingBehaviour>();
			jumper4->setDirection(2);
			// Example of a trigger that interacts with static and kinematic bodies as well as dynamic bodies
			TriggerVolume::Sptr trigger = monkey4->Add<TriggerVolume>();
			trigger->SetFlags(TriggerTypeFlags::Statics | TriggerTypeFlags::Kinematics);
			trigger->AddCollider(BoxCollider::Create(glm::vec3(1.0f)));

			monkey4->Add<TriggerVolumeEnterBehaviour>();
		}
		GameObject::Sptr monkey5 = scene->CreateGameObject("Monkey 5");
		{
			// Set position in the scene
			monkey5->SetPostion(glm::vec3(5.5f, 4.0f, 1.0f));

			// Add some behaviour that relies on the physics body
			RotatingBehaviour::Sptr jumper5 = monkey5->Add<RotatingBehaviour>();
			jumper5->setDirection(3);
			// Create and attach a renderer for the monkey
			RenderComponent::Sptr renderer = monkey5->Add<RenderComponent>();
			renderer->SetMesh(sphere);
			renderer->SetMaterial(boxMaterial);
			// Example of a trigger that interacts with static and kinematic bodies as well as dynamic bodies
			TriggerVolume::Sptr trigger = monkey5->Add<TriggerVolume>();
			trigger->SetFlags(TriggerTypeFlags::Statics | TriggerTypeFlags::Kinematics);
			trigger->AddCollider(BoxCollider::Create(glm::vec3(1.0f)));

			monkey5->Add<TriggerVolumeEnterBehaviour>();
		}
